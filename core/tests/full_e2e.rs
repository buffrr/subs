...
async fn test_certificate_flow(rig: &TestRig, temp_dir: &TempDir) {
    ...
    let (root_cert, handle_cert) = ctx.operator.issue_cert(&handle).await.expect("issue_cert");
    let handle_cert = handle_cert.expect("handle cert");
    assert_temp_cert(&handle_cert, false);

    let verify = ctx.operator.verify_certificate(handle_cert, Some(root_cert)).await
        .expect("verify_certificate");
    assert!(verify.valid);
    ...
    let (root_cert, handle_cert) = ctx.operator.issue_cert(&staged_handle).await
        .expect("issue_cert");
    let handle_cert = handle_cert.expect("handle cert");
...
async fn test_commitment_chain(rig: &TestRig, temp_dir: &TempDir) {
    ...
    // Issue final certificates for handles from both commits
    // This tests that certs work correctly after proving and multiple commits

    // Final cert for handle from first commit
    let handle1 = test_sname(&ctx.handle("user1"));
    let (root_cert, handle_cert) = ctx.operator.issue_cert(&handle1).await.expect("issue_cert user1");
    let handle_cert = handle_cert.expect("handle cert");
    assert_final_cert(&handle_cert);
    let verify = ctx.operator.verify_certificate(handle_cert, Some(root_cert)).await
        .expect("verify_certificate");
    ...
    // Final cert for handle from second commit (after proving)
    let handle3 = test_sname(&ctx.handle("user3"));
    let (root_cert, handle_cert) = ctx.operator.issue_cert(&handle3).await.expect("issue_cert user3");
    let handle_cert = handle_cert.expect("handle cert");
    assert_final_cert(&handle_cert);
    let verify = ctx.operator.verify_certificate(handle_cert, Some(root_cert)).await
        .expect("verify_certificate");
